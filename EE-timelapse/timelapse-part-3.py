...
for pre in sorted(this_camera.previews)[0::steps]:
    ...
    if os.path.isfile(local_filename) == False:
        ...
        auth = een.session.cookies.get_dict()['auth_key']

        url = f"https://een.cloud/framepuller/api/pull_frame/{this_camera.camera_id}/{pre}/{auth}"
        res = requests.get(url)

        url = f"https://een.cloud/framepuller{res.json()['link_to_download']}"
